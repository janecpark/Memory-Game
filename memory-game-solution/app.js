document.addEventListener("DOMContentLoaded", function() {
  ...
  function handleCardClick(e) {
    ...
    if (!card1 || !card2) { //if card1 or card2 doesn't have a value ..
      if (!currentCard.classList.contains("flipped")) { //if the clicked card hasn't already been flipped
        setScore(currentScore + 1); // add 1 to the score
      }
      currentCard.classList.add("flipped"); // then add the class flipped 
      card1 = card1 || currentCard; 
      console.log(card1, currentCard)
      //card1 is currentCard
      //card2 is null
      card2 = currentCard === card1 ? null : currentCard; 
      console.log(card2, currentCard)
    ...
    //before first click, both cards are null
    //card1 is the current card and card2 is null if card1's value is current card
    //2nd click card1 = card1 then card2 will be the current card

    if (card1 && card2) { //if there are values in both card1 and card 2
